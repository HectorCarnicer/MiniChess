...
void Gardner::imprimirTablero()
{
    ...
    for (const auto& pieza : piezas) { //recibes el vector de piezas: mundo.h
        int x, y;
...
void Gardner::inicializa()
{
    ...
    this->nuevaPieza(new Rey(2, 0, BLANCO, TAMANO_TABLERO));
    for (int i = 0; i < TAMANO_TABLERO; ++i) {
        this->nuevaPieza(new Peon(i, 1, BLANCO, TAMANO_TABLERO));
    }
    // Creación de piezas negras
    this->nuevaPieza(new Rey(2, 4, NEGRO, TAMANO_TABLERO));
    for (int i = 0; i < 5; ++i) {
...
void Gardner::nuevaJugada(Color turnoActual)
{
    ...
    for (int i = 0; i < piezas.size(); ++i) {
        if (piezas[i]->obtenerColor() == turnoActual) {
            std::cout << i + 1 << ". " << piezas[i]->nombreDeClase() << " ";
    ...
    if (eleccion > 0 && eleccion <= piezas.size() && piezas[eleccion - 1]->obtenerColor() == turnoActual) {
        ...
        std::cout << "Ingrese la nueva posicion X (0 a 4): ";
        std::cin >> nuevoX;
        std::cout << "Ingrese la nueva posicion Y (0 a 4): ";
        std::cin >> nuevoY;

        // Verificar si la posición está ocupada antes de mover la pieza
        if (!posicionOcupada(nuevoX, nuevoY) && caminoLibre(piezaSeleccionada, nuevoX, nuevoY)) {  //revisa las posiciones de todas las piezas y que no choque con otras piezas
